/**
 * Entry point for the Charlaton WebRTC application.
...
io.on("connection", (socket) => {
  ...
  async function emitUsers(roomId: string) {
    ...
    console.log(
      `[EMIT_USERS] Emitting ${users.length} users for room ${roomId}`
    );
  ...
  socket.on("join_room", async ({ roomId, success }) => {
    ...
    try {
      console.log(
        `[ROOM] 👤 User ${user.email} attempting to join room ${roomId} in WebRTC server`
      );
      ...
      console.log(
        `[ROOM] ✅ User ${user.email} (${userId}) joined WebRTC room ${roomId}`
      );
  ...
  socket.on("webrtc_offer", async ({ roomId, targetUserId, sdp }) => {
    ...
    const targetSocket = sockets.find(
      (s) =>
        (s.data.userId === targetUserId || s.data.user?.id === targetUserId) &&
        s.id !== socket.id
    );
  ...
  socket.on(
    ...
    async ({ roomId, targetUserId, candidate }) => {
      ...
      const userId = user.id;
      console.log(
        `[ICE] Forwarding ICE candidate from ${userId} to ${targetUserId}`
      );
      ...
      const targetSocket = sockets.find(
        (s) =>
          (s.data.userId === targetUserId ||
            s.data.user?.id === targetUserId) &&
          s.id !== socket.id
      );
  ...
  socket.on("disconnect", async () => {
    ...
    if (roomId) {
      ...
      const userId = user.id;
      console.log(
        `[DISCONNECT] User ${user.email} (${userId}) left room ${roomId}`
      );
...
httpServer.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `[SERVER] 📱 Accessible on local network at http://192.168.20.123:${PORT}`
  );
